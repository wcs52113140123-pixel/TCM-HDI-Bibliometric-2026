"""
Generate a unified inventory CSV of all raw data files (WoS + Scopus + OpenAlex + PubMed).
...
# WoS partial 2026
for f in sorted((repo_root / 'data' / 'raw' / 'wos_partial2026').glob('wos_partial2026_*.txt')):
...
if scopus_main.exists():
    ...
    records.append({
        'database': 'Scopus', 'purpose': 'main', 'batch': 1,
        'file_path': str(scopus_main.relative_to(repo_root)).replace(os.sep, '/'),
        'record_range_start': 1, 'record_range_end': count, 'record_count': count,
...
if scopus_p.exists():
    ...
    records.append({
        'database': 'Scopus', 'purpose': 'partial_2026', 'batch': 1,
...
if openalex_main.exists():
    ...
    records.append({
        'database': 'OpenAlex', 'purpose': 'main', 'batch': 1,
...
if openalex_p.exists():
    ...
    records.append({
        'database': 'OpenAlex', 'purpose': 'partial_2026', 'batch': 1,
...
# PubMed main
pubmed_main = repo_root / 'data' / 'raw' / 'pubmed' / 'pubmed_records.jsonl'
if pubmed_main.exists():
    count = count_jsonl_lines(pubmed_main)
    records.append({
        'database': 'PubMed', 'purpose': 'main', 'batch': 1,
        'file_path': str(pubmed_main.relative_to(repo_root)).replace(os.sep, '/'),
        'record_range_start': 1, 'record_range_end': count, 'record_count': count,
        'file_size_mb': round(pubmed_main.stat().st_size / 1024 / 1024, 2),
        'format': 'jsonl',
    })

# PubMed partial
pubmed_p = repo_root / 'data' / 'raw' / 'pubmed_partial2026' / 'pubmed_partial2026_records.jsonl'
if pubmed_p.exists():
    count = count_jsonl_lines(pubmed_p)
    records.append({
        'database': 'PubMed', 'purpose': 'partial_2026', 'batch': 1,
        'file_path': str(pubmed_p.relative_to(repo_root)).replace(os.sep, '/'),
        'record_range_start': 1, 'record_range_end': count, 'record_count': count,
        'file_size_mb': round(pubmed_p.stat().st_size / 1024 / 1024, 2),
        'format': 'jsonl',
    })
